/**
 * Hole-puncher: reduce compressed binary size by zeroing unused ICU data.
 *
 * "Punches holes" in binaries by zeroing unused ICU data entries inside the
 * embedded ICU data blob. These zeroed regions compress to nearly nothing,
 * reducing compressed download size by ~24%.
 *
 * Works with any binary that embeds ICU common data (Bun, Node.js SEA,
 * or any executable linked against ICU with full data).
 *
 * How it works:
 ...
 * These contain core data needed by the ICU runtime.
 */
const KEEP_PREFIXES = [
	"root",
	"en",
 ...
 */
export function findIcuBlob(buf: Buffer): number {
 ...
 */
export function estimateLastEntrySize(entries: IcuEntry[]): number {
	if (entries.length < 2) {
		return 4096;
	}
 ...
 */
export function parseIcuToc(buf: Buffer, blobOffset: number): IcuScanResult {
 ...
 *
 * Safe to remove:
 * - `.cnv` files: legacy charset converters (never used in JS runtimes)
 ...
 */
export function shouldRemoveEntry(name: string): boolean {
 ...
 */
export function holePunch(buf: Buffer, scan: IcuScanResult): HolePunchStats {
 ...
 * when the ICU blob has an unexpected layout, so callers can skip
 * hole-punch gracefully instead of crashing.
 *
 * @returns Hole-punch statistics, or null if no ICU data was found/parseable
 */
export function processBinary(filePath: string): HolePunchStats | null {
...
export function runCli(
	...
	if (filePaths.length === 0) {
		return {
			error: "Usage: hole-puncher [--verbose] <binary-path> ...",
		};
	}
